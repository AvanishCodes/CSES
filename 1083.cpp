...
#define fn for (int i = 0; i < n; i++)       // Condensed for loop
...
void testCase()
{
  int n = read(int); // Number of elements
  // cout << x;
  VI arr(n + 1, 0);
  n -= 1;
  fn
  ...
  // fn cout << arr[i] << ' ';
  // cout << '\n';
  // cout << (lower_bound(arr.begin(), arr.end(), 20) - arr.begin()) - 2;
  fn if (arr[i] == 0 && i > 0) printf("%d", i);
