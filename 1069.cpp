...
void testCase()
{
  ...
  int currentMaxCount = 1, currentPosition = 0;
  while (current != '\0')
  {
    int tempCount = 1;
    bool flag = false;
    while (inputString[currentPosition + 1] == current)
    {
      flag = true;
      tempCount += 1;
      currentPosition += 1;
    }
    currentMaxCount = max(currentMaxCount, tempCount);
    if (!flag)
      currentPosition += 1, current = inputString[currentPosition];
  }
  cout << currentMaxCount;
  return;
}
